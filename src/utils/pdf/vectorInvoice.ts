...
export async function generateInvoicePDFVector(data: VectorInvoiceData) {
  ...
  if (isProforma && (invoiceData.paymentTerms || invoiceData.bankName || invoiceData.remarks)) {
    ...
    doc.rect(paymentTermsX, totalsY, paymentTermsWidth, sectionHeight, 'F');
    doc.setDrawColor(0, 0, 0);
    doc.setLineWidth(0.3);
    doc.rect(paymentTermsX, totalsY, paymentTermsWidth, sectionHeight, 'S');
    
    // Payment terms title
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text('PAYMENT TERMS & CONDITIONS', paymentTermsX + paymentTermsWidth/2, totalsY + 6, { align: 'center' });
    
    // Divider line
    doc.setDrawColor(150, 150, 150);
    doc.setLineWidth(0.2);
    doc.line(paymentTermsX + 5, totalsY + 9, paymentTermsX + paymentTermsWidth - 5, totalsY + 9);
    
    // Payment terms content
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    let contentY = totalsY + 15;
    
    // Payment terms with fixed account number
    doc.text('Payment Terms: 100% Advance', paymentTermsX + 3, contentY);
    contentY += 4;
    doc.text('Account No: EG230014002300001029355429202', paymentTermsX + 3, contentY);
    contentY += 4;
    
    // Bank information
    if (invoiceData.bankName || invoiceData.accountNo) {
      doc.setFont('helvetica', 'bold');
      doc.text('Banking Details:', paymentTermsX + 3, contentY);
      doc.setFont('helvetica', 'normal');
      contentY += 4;
      
      if (invoiceData.bankName) {
        doc.text(`Bank: ${invoiceData.bankName}`, paymentTermsX + 5, contentY);
        contentY += 3;
      }
      
      if (invoiceData.accountNo) {
        doc.text(`Account No: EG230014002300001029355429202`, paymentTermsX + 5, contentY);
        contentY += 3;
      }
    }
    ...
    if (invoiceData.portOfLoading) {
      doc.text(`Port of Loading: ${invoiceData.portOfLoading}`, paymentTermsX + 3, contentY);
      contentY += 3;
    }
    
    if (invoiceData.portOfDestination) {
      doc.text(`Port of Destination: ${invoiceData.portOfDestination}`, paymentTermsX + 3, contentY);
      contentY += 3;
    }
    ...
    if (invoiceData.remarks) {
      doc.setFont('helvetica', 'bold');
      doc.text('Remarks:', paymentTermsX + 3, contentY);
      doc.setFont('helvetica', 'normal');
      contentY += 3;
      
      // Handle long remarks text
      const remarksLines = doc.splitTextToSize(invoiceData.remarks, paymentTermsWidth - 10);
    ...
    doc.setDrawColor(0, 0, 0);
    doc.rect(adjustedTotalsX, totalsY, adjustedTotalsWidth, 35, 'S');
    
    // Totals title
    doc.setFont('helvetica', 'bold');  
    doc.setFontSize(10);
    doc.setTextColor(0, 0, 0);
    doc.text('TOTALS SUMMARY', adjustedTotalsX + adjustedTotalsWidth/2, totalsY + 6, { align: 'center' });
    
    // Individual total lines
    doc.setFont('helvetica', 'normal');
    doc.setFontSize(8);
    doc.text(`Subtotal:`, adjustedTotalsX + 3, totalsY + 12);
    doc.text(`${invoiceData.currencySymbol}${invoiceData.grandTotal.toFixed(2)}`, adjustedTotalsX + adjustedTotalsWidth - 3, totalsY + 12, { align: 'right' });
    
    doc.text(`Tax (${invoiceData.taxPercent}%):`, adjustedTotalsX + 3, totalsY + 17);
    doc.text(`${invoiceData.currencySymbol}${invoiceData.taxAmount.toFixed(2)}`, adjustedTotalsX + adjustedTotalsWidth - 3, totalsY + 17, { align: 'right' });
    
    doc.text(`Discount (${invoiceData.discountPercent}%):`, adjustedTotalsX + 3, totalsY + 22);
    doc.text(`-${invoiceData.currencySymbol}${invoiceData.discountAmount.toFixed(2)}`, adjustedTotalsX + adjustedTotalsWidth - 3, totalsY + 22, { align: 'right' });
    
    // Net total with emphasis
    doc.setDrawColor(0, 0, 0);
    doc.line(adjustedTotalsX + 3, totalsY + 25, adjustedTotalsX + adjustedTotalsWidth - 3, totalsY + 25);
    doc.setFont('helvetica', 'bold');
    doc.setFontSize(10);
  ...
  // Payment terms are now handled in the side-by-side layout above for Proforma invoices
